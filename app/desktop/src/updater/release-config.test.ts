...
describe("alpha release configuration", () => {
  it("keeps app-local published versions fixed at 0.4.1", () => {
    expect(packageJson.version).toBe("0.4.1");
    expect(nativeE2ePackageJson.version).toBe("0.4.1");
    expect(tauriConfig.version).toBe("0.4.1");
    expect(cargoToml).toMatch(/^version = "0\.4\.1"$/m);
  });
