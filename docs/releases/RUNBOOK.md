...
- `scripts/check-release-workflow.mjs` — the contract test hard-codes the release version itself: `releaseVersion`, the `release_tag` default assertion, the changelog path (twice), the `# NeuralNote X.Y.Z ALPHA` heading assertion, and the test's own name. Bumping the workflow without bumping its checker leaves the gate asserting the previous release. **Four of these are regexes.**
  ...
  It also pins **content, not just the version**: every `## ` section heading of the changelog and one distinctive phrase from each section (`assert.match(releaseNotes, /…/)`). Those exist so the published GitHub body cannot silently become a stub or the wrong file, and since every release invents its own section titles they must be re-pointed at the new changelog by hand. Choose phrases specific to this release — a generic one that would match any release retires the check while appearing to keep it.

  **The number of sections changes between releases, so nothing here states one.** 0.4.1 had five and 0.4.2 has six; the assertions are plain arrays with no length check, so a new section is added rather than renamed into an existing slot. This paragraph said "the five section headings" until 0.4.2, which is the same stale-count trap called out below for the workflow's line count — a number in prose that nobody re-derives is a number that goes quietly wrong.

  Pick each phrase so it appears in the `.md` **exactly once** — verify with `grep -c`, and check the hit is in the section you meant. The introduction paraphrases every section, so the obvious summary phrase usually matches twice; `getByText` in the article test throws on that ambiguity and the failure reads as *absence*, which sends you looking for a missing bullet that is actually present.
