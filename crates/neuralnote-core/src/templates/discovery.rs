...
mod tests {
    ...
    /// The crate-wide warning capture. It used to live here; it moved to
    /// `crate::warning_capture` when a second module needed to assert a warning
    /// too, because `log` allows exactly one logger per process and a second
    /// harness would have raced this one for the slot rather than duplicating it.
    use crate::warning_capture as warnings;
