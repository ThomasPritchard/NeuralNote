...
| Requirement installer | malicious archive/binary, race, partial install, an installed executable that cannot run | compiled HTTPS URL and digest, streamed SHA-256, archive entry/type/size limits, archive metadata records skipped before extraction, install locks, atomic publication, a locally compiled executable launched once under a cleared environment before it is kept, and replacement of an already-installed executable permitted only while the advisory lock is held and only when it is proven unusable |
| Installed-executable inspection | a crafted file in app-data crashing or misleading the health check | bounded read of an app-data Mach-O image via a maintained reader, a size ceiling before any parse, no execution of the inspected file, unreadable or non-UTF-8 load-command strings reported as undetermined rather than healthy, shared-cache membership answered by dyld rather than a path prefix |
